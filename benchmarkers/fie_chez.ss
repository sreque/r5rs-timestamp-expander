...
(define benchmark-path (car args))
(define src-files 
  (cond
    [(file-directory? benchmark-path)
      (map (lambda (v)
             (string-append benchmark-path (string (directory-separator)) v))
        (filter (lambda (v)
                  (or (string-ends-with? v ".rkt") (string-ends-with? v ".scm")))
                (directory-list benchmark-path)))]
    [(file-exists? benchmark-path)
     (list benchmark-path)]
    [else
      (errorf #f "provided path does not exist or is not a directory: ~a" benchmark-path)]))

;(display (read-all (car src-files)))
;(display src-files) (newline)
  ...
  (when (not (null? rem))
    (guard 
      (ex
        (#t
        (begin
          (fprintf (console-error-port) "~a failed: ~a" (car rem) ex)
          (loop (cdr rem)))))

    (let* ((path (car rem))
           (result (benchmark (lambda () (expand-file path))))
           (stats (cdr result))
           (expanded (car result)))
      ;(printf "~a\n" expanded)
